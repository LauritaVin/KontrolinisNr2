...
class Course:
    ...
    def generate_report(self):
        for student in self.students:
            ...
            formatted_status = []
            for date, status in attendance_record:
                formatted_status.append(f"{date}: {status}")
                attendance_status = ', '.join(formatted_status)
    ...
    def get_lessons(self):
        for lesson in self.lessons:
            print(f"Lesson Topic: {lesson.topic}, Date: {lesson.date}, Materials: {', '.join(lesson.materials)}")
...
alice.performance_report()
bob.performance_report() # Student: Alice, Course: Mathematics, Grade: A
